...
def trilinear_interpolation(target, corners, sample_color_map):
    ...
    RGBr = []
    for i in range(3):
        temp = (target[i] - corners[i][0]) / (corners[i][1] -
                                              corners[i][0]) if corners[i][0] != corners[i][1] else 0
...
def image_transfer(image, original_p, modified_p, sample_level=16, luminance_flag=False, verbose=False):
    t = time.time()
    # init
